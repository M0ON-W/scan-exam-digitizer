...
2. **ZIP/source-tree fidelity** — Recompute the input archive SHA-256 and compare it with `PROVENANCE.md`; compare every regular-file path under `skills/scan-exam-digitizer/` with the archive after stripping its single top-level directory. Expected: the fixed archive hash, all file paths, and all bytes match except the documented `agents/openai.yaml` product-token normalization.
